 ...
 *
 * 两个作用：
 * 1. 负载均衡，轮询的方式，将消息尽量打到多个broker上
 * 2. 故障延迟，发送消息失败的broker会尽量等10分钟后再次尝试使用
 */
...
public class MQFaultStrategy {
    ...
    public MessageQueue selectOneMessageQueue(final TopicPublishInfo tpInfo, final String lastBrokerName) {
        ...
        if (this.sendLatencyFaultEnable) {
            try {
                ...
                // 上面没有找到可用的，就从曾经失败或成功的里取一个(不管现在是否可用)，
                // 之所以包含成功的可能是刚发送的broker还不能再次使用
    ...
    public void updateFaultItem(final String brokerName, final long currentLatency, boolean isolation) {
        if (this.sendLatencyFaultEnable) {
            // 根据currentLatency获取不可用的时长，如果发送失败延迟10分钟重试(30000->10)
    ...
    private long computeNotAvailableDuration(final long currentLatency) {
        // 从尾向前找第一个小于currentLatency的latencyMax，并取notAvailableDuration对应的不可用时长
